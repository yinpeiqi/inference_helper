...
class HGTLayer(nn.Module):
    ...
    def forward(self, G, x_author, x_field_of_study, x_institution, x_paper):
        with G.local_scope():
            h = {"author": x_author, "field_of_study": x_field_of_study, "institution": x_institution, "paper": x_paper}
            h_dst = {}
            ...
            return new_h["author"], new_h["field_of_study"], new_h["institution"], new_h["paper"]
...
class HGT(nn.Module):
    def __init__(self, G, node_dict, edge_dict, n_inp, n_hid, n_out, n_layers, n_heads, use_norm = True):
        ...
        self.out4 = nn.Linear(n_hid, n_out)
    ...
    def forward(self, G, x_author, x_field_of_study, x_institution, x_paper):
        ...
        x_field_of_study = self.adapt_ws[1](x_field_of_study)
        x_field_of_study = F.gelu(x_field_of_study)

        x_institution = self.adapt_ws[2](x_institution)
        x_institution = F.gelu(x_institution)

        x_paper = self.adapt_ws[3](x_paper)
        x_paper = F.gelu(x_paper)

        for i in range(self.n_layers):
            x_author, x_field_of_study, x_institution, x_paper = self.gcs[i](G[i], x_author, x_field_of_study, x_institution, x_paper)
        return self.out1(x_author), self.out2(x_field_of_study), self.out3(x_institution), self.out4(x_paper)
